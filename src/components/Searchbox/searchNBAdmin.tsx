...
interface Props {
  ...
  textColor?: string;
  placeholder?: string;
  onSelect: (selectedPerson: SearchType | null) => void; // Add onSelect prop
}

export default function SearchNBAdmin({
  searchArray,
  onSelect, linkTo,
  bg,
  textColor,
  ...
  const handleSearchChange = (admin: any) => {
    setQuery(query || (admin?.FullName ?? admin?.name)?.trim());
  ...
  const handleInputXbutton = () => {
    //setTextSearch('')
    setQuery('');
    onSelect(null); // Clear the selected person
  };
    ...
    <>
      ...
      >
        <div className="relative mt-1 mx-auto max-w-7xl w-full">
          <div
            className={`relative w-full cursor-default overflow-hidden text-left shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm ${
              bg ?? 'bg-[#38404b]'
            } lg:h-16 h-12 flex justify-between items-center px-4 rounded-[0.99063rem] box-border
            `}
          >
            <p
              className={`text-base text-[${colorText}] z-20 lg:w-7 w-5 cursor-pointer`}
            >
              {query === '' || selected === '' ? (
                <FaSearch
                  title="search"
                  className={`text-base text-[#CBCBCB] `}
                />
              ) : (
                <BiArrowBack
                  title="go back"
                  onClick={handleArrowBackButton}
                  className={`md:text-2xl text-xl text-[#CBCBCB] `}
                />
              )}
            </p>
            <Combobox.Input
              ...
              onChange={(event) =>
                setQuery(
                  event.target.value.trim() ||
                    (selected?.FullName ?? selected?.name).trim()
                )
              }
            />
            {filteredPeople == null || filteredPeople == undefined ? (
              <FaSpinner className="animate-spin text-lg text-gray-200" />
            ) : (
              <Combobox.Button className="absolute inset-y-0 right-0 flex items-center pr-4">
                {query === '' || selected === '' ? (
                  ...
                  />
                ) : query.length > 1 || selected.length > 1 ? (
                  <HiMiniXMark
                    className="h-5 w-5 text-gray-400"
                    aria-hidden="true"
              ...
              {filteredPeople?.length === 0 && query !== '' ? (
                ...
                filteredPeople?.map((person) => (
                  <Combobox.Option
                    ...
                    onClick={() =>
                      onSelect(person)
                    } /* Pass the selected person to the parent */
                  >
                    {({ selected, active }) => (
                      <>
                        <span
