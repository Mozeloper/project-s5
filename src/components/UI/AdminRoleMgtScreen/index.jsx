...
export default function AdminRoleMgt({ handleAdminRole, adminId, roles, name }) {
  ...
  const [adminRoles, setAdminRoles] = useState({
    ...
    useEffect(() => {
      if (roles && roles.length > 0) {
        ...
        // If no roles are provided, fetch roles using the adminId
  ...
  const handleRoleToggle = (role) => {
    ...
    // Replace this with your actual API call logic
    updateAdminRoles(adminId, role);
  };

  const updateAdminRoles = async (adminId, role) => {
    // Simulate an API call to update the admin's roles
      try {
        setIsLoading(role);
    ...
    console.log(`API call to update ${role} for admin with ID ${adminId}`);
  };
  return (
    <>
